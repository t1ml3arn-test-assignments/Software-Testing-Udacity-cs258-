...
2. Branch coverage
	A branch is covered if it executes both ways, i.e when the condition is true and when it is false. There also can be more `else` conditions in the branch
4. Loop coverage (to fully cover loop it is needed: no loop, 1 loop, more than 1 loop)
5. MC/DC - modified conditions / decision = branch + conditions gets all possible values + every condition independently affects the result
	This metric is used in critical software. It might show that there are conditions which have no effect on the results and thus one who wrote the code has no understanding what is going on.
6. Path coverage
```py
def foo(a, b):
	if a:
	...
	return do_something()
```
In the example above there are 4 paths (2^2). For complex statements the number of paths will be much greater. The goal is to exploit control structure of a programm.
...
3. Inadequate test suite
	Either test code should be improved to achive more coverage, or it can be left at it is since we don't need to get 100% coverage.
...
- [The theory of Path testing](https://testerstories.com/2014/06/path-testing-the-theory/)